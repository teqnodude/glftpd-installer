...
# Debug detection - safer approach
DEBUG=0
for arg in "$@"
do
...
func_tail()
{
    if tail -r /dev/null >/dev/null 2>&1
    then
...
do
    ...
    for dn in $( ${SITEWHO} --raw | grep "${release}" | grep '"DN"' | awk -F\" '{ print $12 }' )
    do
...
if [[ "${bwavg}" -ne "0" ]] || [[ "${SHOW_ALWAYS}" -eq "1" ]]
then
    ...
    then
        ...
        for line in $( func_tail | grep -v "${PREDIR}" | grep " o " | grep "${release}" | awk '{ print $8, $14, $15 }' )
        do
    ...
    then
        ...
        echo "DEBUG: traf_total=$traf_total bwavgtext=$bwavgtext"
