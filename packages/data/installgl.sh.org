...
if [ "$netd" = "i" ]; then
    ...
    { grep -v ^${servicename} /etc/inetd.conf
        echo "$servicename stream tcp nowait root $tcpd $jaildir$glroot/bin/glftpd-full-static $dummyflag -l -o -i -e -I -n 0 -b -x -r /glftpd/etc/glftpd.conf" | tr -s ' '
    } > /etc/inetd.conf.new
...
service $servicename
{
    ...
    wait            = no
    user            = root
    server          = ${tcpd:-$jaildir$glroot/bin/glftpd-full-static}
    server_args     = ${tcpd:+$jaildir$glroot/bin/glftpd-full-static} -l -o -i -e -I -n 0 -b -x -r /glftpd/etc/glftpd.conf
}
EOF
    echo "Done."
...
service "$servicename"
{
	...
	port "$servicename";
	user "root";
	exec "${tcpd:+$jaildir$glroot/bin/glftpd-full-static} $dummyflag -l -o -i -e -I -n 0 -b -x -r /glftpd/etc/glftpd.conf";
	server "${tcpd:-$jaildir$glroot/bin/glftpd-full-static}";
}
EOF
    echo "Done."
...
[Service]
ExecStart=-$jaildir$glroot/bin/glftpd-full-static -l -o -i -e -I -n 0 -b -x -r /glftpd/etc/glftpd.conf
