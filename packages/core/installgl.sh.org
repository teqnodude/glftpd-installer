...
mknod -m666 "$glroot/dev/full" c 1 7
mknod -m666 "$glroot/dev/urandom" c 1 9
...
# Check /etc/services
if [ ! -z "$(grep -e "^${servicename}$" /etc/services 2>/dev/null)" ]; then
    echo "Warning: Service '${servicename}' already exists. It will be updated."
fi

# Check /etc/services.d/ if it exists
if [ -d /etc/services.d ]; then
    # Use a loop to avoid wildcard expansion issues
    for file in /etc/services.d/*; do
        if [ -f "$file" ] && [ ! -z "$(grep -e "^${servicename}$" "$file" 2>/dev/null)" ]; then
            echo "Warning: Service '${servicename}' already exists in $file. It will be updated."
            break
        fi
    done
fi
...
echo ""
echo -n "Adding glftpd service to /etc/services.d (as ${servicename}) . . . "
[[ ! -d /etc/services.d/ ]] && mkdir /etc/services.d
echo "$servicename   $port/tcp" > /etc/services.d/glftpd
echo "Done."
